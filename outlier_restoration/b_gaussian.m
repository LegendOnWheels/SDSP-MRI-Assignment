...
x_smooth1 = gauss_smooth_cols(x_bad1, badCols1, sigma, filterSize, protectHW);
x_smooth2 = gauss_smooth_cols(x_bad2, badCols2, sigma, filterSize, protectHW);
x_smooth3 = gauss_smooth_cols(x_bad3, badCols3, sigma, filterSize, protectHW);
...
sgtitle('ALC + Gaussian Smoothing in K-Space (coil 1)');
...
% 1-D Gaussian smoothing (along rows ONLY to specified columns)
% Note the protected k-space 

function Y = gauss_smooth_cols(X, badCols, sigma, filterSize, protectHW)
    ...
    % Which flagged columns will we actually smooth?

    toSmooth = badCols(~protect(badCols));
    skipped  = badCols(protect(badCols));

    if ~isempty(skipped)
        fprintf('[gauss] Protected (not smoothed) cols near DC: %s\n', mat2str(skipped));
    end
    if isempty(toSmooth), return; end

    % Build separable 1-D Gaussian kernel along rows

    if mod(filterSize,2)==0, filterSize = filterSize+1; end
    h = fspecial('gaussian', [filterSize, 1], sigma);

    % Smooth only the selected columns

    for c = toSmooth(:).'
        Y(:,c) = imfilter(real(X(:,c)), h, 'replicate') + 1i*imfilter(imag(X(:,c)), h, 'replicate');
    end
