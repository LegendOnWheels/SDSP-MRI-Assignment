...
x_smooth1 = lininterp_cols(x_bad1, badCols1);
x_smooth2 = lininterp_cols(x_bad2, badCols2);
x_smooth3 = lininterp_cols(x_bad3, badCols3);
...
sgtitle('ALC + Linear Interpolation in K-Space (coil 1)');
...
function Y = lininterp_cols(X, badCols)
    Y = X; Nx = size(X,2);
    good = true(1,Nx); good(badCols) = false;
    ...
    for r = 1:size(X,1)
        ...
        if nnz(good)==1
            ...
            F  = griddedInterpolant(gxi, yr(good), 'linear', 'nearest');
            yi = F(xi);
            Y(r,badCols) = yi(badCols);
        end
    end
end
